...
class IcmpHelperLibrary:
    ...
    class IcmpPacket:
        ...
        def __validateIcmpReplyPacketWithOriginalPingData(self, icmpReplyPacket):
            ...
            # the packet can be confirmed
            if(self.__packetSequenceNumber == icmpReplyPacket.getIcmpSequenceNumber() and
            self.__dataRaw == icmpReplyPacket.getIcmpData() and self.__packetIdentifier == icmpReplyPacket.getIcmpIdentifier()):
                icmpReplyPacket.setIsValidResponse(True)

            else:
                if(self.__packetIdentifier != icmpReplyPacket.getIcmpIdentifier()):
                    icmpReplyPacket.setIsValidIdentifier(False)
                print("the packet identifier is invalid")
                print("actual: ", icmpReplyPacket.getIcmpIdentifier())
                print("expected: ", self.__packetIdentifier)
            
                # this if else statement checks whether the data is correct
                if(self.__dataRaw != icmpReplyPacket.getIcmpData()):
                    icmpReplyPacket.setIsValidData(False)
                    print("the packet data is invalid")
                    print("actual: ", icmpReplyPacket.getIcmpData())
                    print("expected: ", self.__dataRaw)

                # this if else statement checks whether packet's sequence number is correct
                if(self.__packetSequenceNumber != icmpReplyPacket.getIcmpSequenceNumber()):
                    icmpReplyPacket.setIsValidSeqNum(False)
                    print("the packet sequence number is invalid")
    ...
    class IcmpPacket_EchoReply:
        ...
        def isValidSeqNum(self):
            return self.__IcmpSeqNum_IsValid

        def getRttValues(self):
            return self.__rttValues
        ...
        def setIsValidIdentifier(self, boolean):
            self.__IcmpIdentifier_IsValid = boolean
        
        def setIsValidData(self, boolean):
            self.__IcmpData_IsValid = boolean

        def setIsValidSeqNum(self, boolean):
            self.__IcmpSeqNum_IsValid = boolean

        def addRttValue(self, value):
            self.__rttValues.append(value)
        ...
        def printResultToConsole(self, ttl, timeReceived, addr):
            ...
            # if it is not valid, issues are reported
            if(self.isValidResponse() == False):
                # checks whether the data is valid
                if(self.isValidData() == False):
                    print("packet data is invalid")
                    print("actual: ", self.getIcmpData())

                elif(self.isValidSeqNum() == False):
                    print("packet sequence number is invalid")
                    print("actual: ", self.getIcmpSequenceNumber())

                else:
                    print("packet identifier is invalid")
                    print("actual: ", self.getIcmpIdentifier())

            else:
                # if no issues are reported then the packet is valid
    ...
    def __sendIcmpTraceRoute(self, host):
        ...
        # runs until the while loop is not 1
        while(x):
            icmpPacket = IcmpHelperLibrary.IcmpPacket()
            icmpPacket.setTtl(y+1)
            randomIdentifier = (os.getpid() & 0xffff)      
            packetIdentifier = randomIdentifier
            packetSequenceNumber = y   
            icmpPacket.buildPacket_echoRequest(packetIdentifier, packetSequenceNumber) 
            icmpPacket.setIcmpTarget(host)
            icmpPacket.sendEchoRequest()

            if((icmpPacket.getIcmpCode() == 0) and (icmpPacket.getIcmpType() != 0)):
                # terminates the while loop from running
                x = 0

            elif((icmpPacket.getIcmpCode() != 0) and (icmpPacket.getIcmpType() == 0)):
                # terminates the while loop from running
                x = 0
    ...
    def traceRoute(self, targetHost):
        ...
        print("your trace has been completed")
